...
def save_checkpoint(path: Path, policy: ActorCritic, update: int, total_steps: int) -> None:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--save-dir", type=Path, default=_ROOT / "runs" / "ppo_v0")
...
def main() -> None:
    ...
    while total_steps < args.total_timesteps:
        ...
        if update % args.save_every == 0:
            ...
            save_checkpoint(ckpt, policy, update, total_steps)
            print(f"saved {ckpt}", flush=True)

    save_checkpoint(args.save_dir / "checkpoint_final.pt", policy, update, total_steps)
